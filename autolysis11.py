...
def analyze_visualization_with_llm(image_path,report, data,advanced_stats):
    ...
    try:
        ...
        - Summary statistics: {report.get('summary', 'N/A')}
        - Correlation matrix: {report.get('correlation_matrix', 'N/A')}
        - Analyze this visualization for insights.
        """
        # Prepare the payload
        payload = {
            "model": "gpt-4o-mini",
...
def analyze_visualization_with_llm1(image_paths, report, data, advanced_stats):
    """
    Analyzes one or more visualizations using an LLM and returns insights.

    Args:
        image_paths (str or list): Path(s) to the visualization image(s).
    ...
    if isinstance(image_paths, str):
        image_paths = [image_paths]  # Convert single path to a list for uniform processing
        ...
        for image_path in image_paths:
            ...
            payload = {
                ...
                "messages": [
                    ...
                    {
                        "role": "user",
                        "content": f"Analyze the visualization: {os.path.basename(image_path)}. "
                                   f"Context from the analysis report: {report.get('summary', 'No summary available.')}. "
                                   f"Advanced statistics: {advanced_stats}."
                    }
                ]
            }
            ...
            insights[image_path] = response.json()["choices"][0]["message"]["content"]

    except Exception as e:
        for image_path in image_paths:
            print(f"Error analyzing visualization {image_path}: {e}")
            insights[image_path] = "No insight generated due to an unexpected error."

    return insights

def process_visualizations_with_llm1(images,report,data,advanced_stats):
    insights = []
    for img in images:
        print(img)
        insight = analyze_visualization_with_llm1(images,report, data,advanced_stats)
        if insight:
...
def process_visualizations_with_llm(images, report, data, advanced_stats):
    ...
    for image_path in images:
        try:
            ...
            insight = analyze_visualization_with_llm(image_path, report, data, advanced_stats)
...
def generate_readme(story, images,output_folder,report,visualization_insights):
    ...
    with open(readme_path, "w") as f:
        ...
        for img, insight in visualization_insights:
...
def main():
    ...
    user_config = {
        "max_images": 6,  # User-specific overrides
        "max_rows": 5,
    }
    config = user_config
    ...
    visualization_insights = process_visualizations_with_llm1(compressed_images,report, data,advanced_stats)
    # Generate insights
